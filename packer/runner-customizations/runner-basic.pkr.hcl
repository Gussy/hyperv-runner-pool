...
# Extends hv-packer's Windows Server 2022 base with GitHub Actions Runner
...
source "hyperv-iso" "runner-basic" {
...
build {
  name = "runner-basic"
  sources = ["source.hyperv-iso.runner-basic"]
  ...
  # CUSTOM: Install basic development tools
  provisioner "powershell" {
    elevated_password = local.elevated_password
    elevated_user     = local.elevated_user
